<!--
  负责：展示按真实平台/会话类型分组的折叠列表，并承载状态 Tabs。
  不负责：详情渲染与消息展示。
-->
<script setup lang="ts">
import { useDateFormat } from '@vueuse/core'
import { AlertTriangle, Bot, ChevronDown, ChevronRight, Clock3, Layers3, Pin, Sparkles } from 'lucide-vue-next'
...
watch(
  ...
  () => {
    openGroups.value = props.groups.map(group => group.key)
  },
  { immediate: true },
)

const decoratedGroups = computed(() => props.groups.map(group => ({
  ...group,
...
<template>
  <aside class="flex min-h-0 flex-col border-r border-border/70 bg-card/35 xl:min-w-[320px] xl:max-w-[360px]">
    <div class="border-b border-border/70 px-3 py-2">
      <div class="flex items-center justify-between">
        ...
        <Badge variant="outline" class="font-mono text-[10px] uppercase tracking-wide" title="真实数据源：~/.hermes">
    ...
    <ScrollArea class="min-h-0 flex-1">
      <div v-if="isLoading" class="p-4 text-xs text-muted-foreground">
        读取 ~/.hermes…
      </div>
      ...
      <div v-else class="space-y-2 p-2">
        ...
        >
          <button
            class="flex w-full items-center gap-2 px-2.5 py-2 text-left text-xs text-muted-foreground transition-colors hover:bg-muted/25"
            :title="group.description"
            @click="toggleGroup(group.key)"
          >
            <component :is="isGroupOpen(group.key) ? ChevronDown : ChevronRight" class="size-3.5 shrink-0" />
            <span class="truncate font-medium text-foreground">{{ group.label }}</span>
            <span class="ml-auto font-mono text-[10px]">{{ group.sessions.length }}</span>
          </button>

          <div v-show="isGroupOpen(group.key)" class="space-y-1 border-t border-border/60 p-1.5">
            <button
            ...
            >
              <component :is="statusMeta[session.status].icon" class="size-3.5 shrink-0" :title="statusMeta[session.status].title" />
              ...
              <AlertTriangle v-else-if="session.issueCount" class="size-3 shrink-0 text-amber-300" :title="`${session.issueCount} 条异常`" />
            </button>
          </div>
        </section>

        <div v-if="decoratedGroups.length === 0" class="px-3 py-8 text-center text-xs text-muted-foreground">
          当前筛选下没有会话。
        </div>
      </div>
    </ScrollArea>
