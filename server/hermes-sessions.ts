...
function deriveToolPreview(parsed: Record<string, unknown> | null, raw: string) {
  if (parsed) {
    for (const key of ['message', 'output', 'error', 'summary', 'content']) {
...
function deriveCommandOutput(parsed: Record<string, unknown> | null) {
  if (!parsed) {
    return ''
  }
...
function buildToolCallEntry(
  ...
  return {
    ...
    name,
    preview: deriveToolPreview(parsed, content),
    rawJson,
    commandOutput,
